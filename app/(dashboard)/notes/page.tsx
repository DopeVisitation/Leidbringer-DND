...
export default function NotesPage() {
  ...
  const gmId = isGM ? user?.id : players.find((p) => p.role === 'gm')?.id
  const chatPlayerId = isGM ? selectedPlayerId : user?.id

  const { messages, sendMessage } = usePrivateMessages(
    chatPlayerId ?? '',
    gmId ?? ''
  )
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto h-[calc(100vh-80px)] md:h-screen flex flex-col gap-4">
      <div>
      ...
      {/* GM: player selector for chat */}
      {isGM && activeTab === 'chat' && players.length > 0 && (
