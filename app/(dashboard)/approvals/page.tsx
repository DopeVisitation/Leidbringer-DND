...
import {
  ...
  Package, MessageSquare, ChevronDown, ChevronUp, Search, Filter,
} from 'lucide-react'
...
export default function ApprovalsPage() {
  ...
  const applyFilters = (list: ApprovalRequest[]) => list.filter(r => {
    const q = searchQ.toLowerCase()
    const matchSearch = !q || r.title.toLowerCase().includes(q) || (r.content ?? '').toLowerCase().includes(q) || (r.requester?.username ?? '').toLowerCase().includes(q)
  ...
  const filteredMyReqs  = applyFilters(myReqs)
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] md:h-screen overflow-hidden">
      ...
      {/* Search + Filter bar */}
      <div className="flex-shrink-0 px-4 py-2 bg-zinc-950 border-b border-zinc-800 flex flex-wrap gap-2 items-center">
        <div className="relative flex-1 min-w-40">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 bg-zinc-800 border border-zinc-700 rounded-lg text-xs text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-amber-500"
          />
          {searchQ && (
        ...
        >
          ...
          <option value="item_price">Itempreise</option>
          <option value="general">Anfragen</option>
        </select>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 max-w-2xl mx-auto w-full">
        {loading ? (
            ...
            {gmTab === 'pending' && (
              filteredPending.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-32 gap-2 text-zinc-600">
                  ...
                  <p className="text-sm">{searchQ || filterType !== 'all' ? 'Keine Treffer' : 'Keine offenen Genehmigungen'}</p>
                </div>
              ) : (
                filteredPending.map(r => <RequestCard key={r.id} req={r} showActions={true} />)
              )
            )}
            {gmTab === 'history' && (
              filteredHistory.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-32 gap-2 text-zinc-600">
                  ...
                  <p className="text-sm">{searchQ || filterType !== 'all' ? 'Keine Treffer' : 'Noch keine bearbeiteten Anfragen'}</p>
                </div>
              ) : (
                filteredHistory.map(r => <RequestCard key={r.id} req={r} showActions={false} />)
              )
            )}
          </>
        ) : (
          <>
            ...
            {/* own requests */}
            {filteredMyReqs.length === 0 && targeted.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-32 gap-2 text-zinc-600">
                <Bell className="w-8 h-8 opacity-30" />
                <p className="text-sm">{searchQ ? 'Keine Treffer' : 'Noch keine Anfragen'}</p>
                {!searchQ && (
                  <button
              ...
              <div className="space-y-2">
                {filteredMyReqs.length > 0 && <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">Meine Anfragen</p>}
                {filteredMyReqs.map(r => <RequestCard key={r.id} req={r} showActions={false} />)}
              </div>
            )}
          </>
        )}
